...
function storageCall(aArea, aAction, aKeys, aOptions) {
	...
	return new Promise(function(resolve, reject) {
		...
		var call = function() {
			switch (aAction) {
				case 'clear':
						browser.storage[aArea][aAction](check);
					break;
				case 'set':
						// special processing
						...
						else browser.storage[aArea][aAction](aKeys, check);
					break;
				default:
					browser.storage[aArea][aAction](aKeys, check);
			}
		};
		...
		var check = function(arg1) {
			if (browser.runtime.lastError) {
				...
				else reject(browser.runtime.lastError); // `maxtries` reached
			} else {
				switch (aAction) {
					case 'clear':
					case 'remove':
